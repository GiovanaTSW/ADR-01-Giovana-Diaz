...
public class LoggingPrendaRepository : IPrendaRepository
{
    ...
    {
        _logger.LogInformation("▶ PrendaRepository.GetAllAsync()");
    ...
    {
        _logger.LogInformation("▶ PrendaRepository.GetByIdAsync({Id})", id);
        var result = await _inner.GetByIdAsync(id);
        _logger.LogInformation("◀ PrendaRepository.GetByIdAsync({Id}) → {Found}", id, result != null ? "encontrado" : "null");
        return result;
    }

    public async Task<List<Prenda>> GetByUsuarioIdAsync(int usuarioId)
    {
        _logger.LogInformation("▶ PrendaRepository.GetByUsuarioIdAsync({UsuarioId})", usuarioId);
    ...
    {
        _logger.LogInformation("▶ PrendaRepository.GetDisponiblesAsync({UsuarioId})", usuarioId);
    ...
    {
        _logger.LogInformation("▶ PrendaRepository.GetDisponiblesParaDonarAsync({UsuarioId})", usuarioId);
    ...
    {
        _logger.LogInformation("▶ PrendaRepository.GetNextIdAsync()");
        var result = await _inner.GetNextIdAsync();
        _logger.LogInformation("◀ PrendaRepository.GetNextIdAsync() → {Id}", result);
        return result;
    }

    public async Task AddAsync(Prenda prenda)
    {
        _logger.LogInformation("▶ PrendaRepository.AddAsync({Nombre})", prenda.Nombre);
        await _inner.AddAsync(prenda);
        _logger.LogInformation("◀ PrendaRepository.AddAsync({Nombre}) → OK", prenda.Nombre);
    }

    public async Task DeleteAsync(int id)
    {
        _logger.LogInformation("▶ PrendaRepository.DeleteAsync({Id})", id);
        await _inner.DeleteAsync(id);
        _logger.LogInformation("◀ PrendaRepository.DeleteAsync({Id}) → OK", id);
    }

    public async Task SaveAsync(List<Prenda> prendas)
    {
        _logger.LogInformation("▶ PrendaRepository.SaveAsync({Count} items)", prendas.Count);
        await _inner.SaveAsync(prendas);
        _logger.LogInformation("◀ PrendaRepository.SaveAsync({Count} items) → OK", prendas.Count);
    }
}
